  ...
  try {
    ...
    if (
      ...
      for (const { source, dest } of paths) {
        const rsyncCommand = `rsync -avz --delete -e "ssh -i /app/.ssh/id_rsa" ${source}/* ${config.rsyncRemoteUser}@${config.rsyncRemoteHost}:${dest}/`
        ...
        if (existsSync(uiSourcePath)) {
          ...
          const mkdirParentCommand = `ssh -i /app/.ssh/id_rsa ${config.rsyncRemoteUser}@${config.rsyncRemoteHost} "mkdir -p ${parentDir}"`
          ...
          try {
            ...
            const mkdirDisplayCommand = `ssh -i /app/.ssh/id_rsa ${config.rsyncRemoteUser}@${config.rsyncRemoteHost} "mkdir -p ${displayRemotePath}"`
