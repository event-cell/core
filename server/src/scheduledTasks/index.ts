  ...
  try {
    ...
    if (
      ...
      for (const { source, dest } of paths) {
        const rsyncCommand = `rsync -avz --delete -e "ssh -i ${config.rsyncSshKeyPath}" ${source}/* ${config.rsyncRemoteUser}@${config.rsyncRemoteHost}:${dest}/`
        ...
        if (existsSync(uiSourcePath)) {
          ...
          const mkdirParentCommand = `ssh -i ${config.rsyncSshKeyPath} ${config.rsyncRemoteUser}@${config.rsyncRemoteHost} "mkdir -p ${parentDir}"`
          ...
          try {
            ...
            const mkdirDisplayCommand = `ssh -i ${config.rsyncSshKeyPath} ${config.rsyncRemoteUser}@${config.rsyncRemoteHost} "mkdir -p ${displayRemotePath}"`
            ...
            const uiRsyncCommand = `rsync -avz --delete -e "ssh -i ${config.rsyncSshKeyPath}" ${uiSourcePath}/* ${config.rsyncRemoteUser}@${config.rsyncRemoteHost}:${displayRemotePath}/`
